 ...
 * same `exports` map. It also proves that BullMQ types re-exported by the
 * server barrel resolve through the library. This module is intentionally
 * runtime-inert and is replaced by real NestJS wiring in a later stage.
 * @layer app/probe
 */
...
/**
 * Type-level proof that BullMQ types re-exported by the server barrel resolve
 * through the library, so consumers never import `bullmq` directly. The `Job`
 * type is reserved for real processor and inspection typings in a later stage.
 */
export type ReexportedJobProbe = Job
